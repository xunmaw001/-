...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 7% 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>画师账号</div>
        <el-input v-model="formSearch.huashizhanghao" placeholder="画师账号" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
      ...
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>接稿状态</div>
        <el-select v-model="formSearch.jiegaozhuangtai" placeholder="请选择接稿状态" :clearable="true">
          <el-option v-for="(item, index) in jiegaozhuangtaiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('huashi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#ffc303","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/huashiAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"40px 0 10px","minWidth":"1050px","background":"none"}'>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 7%","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"margin":"0 0 40px","flexWrap":"wrap","background":"rgba(255,255,255,.6)","display":"flex","width":"49%","fontSize":"0","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"border":"2px dashed #333","padding":"10px","objectFit":"cover","borderRadius":"100%","display":"inline-block","width":"285px","height":"285px"}' v-if="item.zhaopian && item.zhaopian.substr(0,4)=='http'" :src="item.zhaopian" class="image" />
				<img :style='{"border":"2px dashed #333","padding":"10px","objectFit":"cover","borderRadius":"100%","display":"inline-block","width":"285px","height":"285px"}' v-else :src="baseUrl + (item.zhaopian?item.zhaopian.split(',')[0]:'')" class="image" />
				<div class="item-info" :style='{"width":"calc(100% - 285px)","padding":"10px 0px","overflow":"hidden","display":"inline-block","height":"100%"}'>
					<div :style='{"padding":"0 40px 0 0","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"right","background":"url(http://codegen.caihongy.cn/20230112/71572c80be344dfc9b3c7ebecf07a4c5.png) no-repeat center top / 100% 100%","lineHeight":"50px","fontSize":"18px","textOverflow":"ellipsis","height":"68px"}' class="name ">{{item.huashixingming}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.huashizhanghao != '') searchWhere.huashizhanghao = '%' + this.formSearch.huashizhanghao + '%';
        if (this.formSearch.huashixingming != '') searchWhere.huashixingming = '%' + this.formSearch.huashixingming + '%';
        if (this.formSearch.jiegaozhuangtai != '') searchWhere.jiegaozhuangtai = this.formSearch.jiegaozhuangtai;
        this.$http.get('huashi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/huashiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
